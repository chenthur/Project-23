...
def getLabelOfAccount(label, label_type="account", input_type='single'):
    ...
    # driver.get(baseUrlLabel.format(label_type, label, 0, index))
    ...
    for table_index, subcat_id in enumerate(subcat_id_list):
        index = 0  # Initialize start index at 0
        driver.implicitly_wait(5)
        print(baseUrlLabel.format(label_type, label, subcat_id, index))
        driver.get(baseUrlLabel.format(label_type, label, subcat_id, index))
        time.sleep(10)  # TODO: allow customization by args
        ...
        while (True):
            ...
            try:
                ...
                addressList.append('end_of_a_table')
    ...
    if (input_type == 'single'):
        ...
        if (endOrContinue == 'exit'):
            ...
            getLabelOfAccount(endOrContinue, label_type=label_type)


# Retrieve label information and saves as JSON/CSV
def getLabelOfToken(label, label_type="token", input_type='single'):
    ...
    driver.get(baseUrlLabel.format(label_type, label, 'undefined', index))
    # print(baseUrlLabel.format(label_type, label, 'undefined', index))
    # driver.get(baseUrlLabel.format(label_type, label, 0, index))
    driver.implicitly_wait(10)
    ...
    # Loop through elems and append all values to subcat_id_list
    for elem in elems:
        elemVal = elem.get_attribute("data-sub-category-id")
        # print(elem.text,elemVal,elem.get_attribute("class")) # Used for debugging elements returned
        if (elemVal is not None):
            subcat_id_list.append(elemVal)
    ...
    for table_index, subcat_id in enumerate(subcat_id_list):
        ...
        while (True):
            ...
            try:
                ...
                addressList = []
                addrIndex = len(baseUrl + '/token/')
                for elem in elems:
                    href = elem.get_attribute("href")
                    # print(href)
                    # if (href.startswith('baseUrl/address/')):
                    if (href.startswith(baseUrl +'/token/')):
                        if href[addrIndex:] not in addressList:
                            addressList.append(href[addrIndex:])
...
def getAllLabels():
    ...
    #     getLabelOfAccount(label, 'account', 'all')
    ...
    for label in labels:
        ...
        if label[0] > 'g':
            getLabelOfToken(label, 'token', 'all')
        time.sleep(1)  # Give 1s interval to prevent RL
...
ignore_list = ['eth2-depositor', 'gnosis-safe-multisig', 'liqui.io', 'education', 'electronics',
               ...
               'remittance+', 'defi', 'gambling']  # Bugged remittance+ bad link, accidental + it seems
...
#         ChromeDriverManager().install()))
    ...
    if (retrievalType == 'all'):
        ...
        # getLabelOfAccount(singleLabel, 'account')
        getLabelOfToken(singleLabel, 'token')